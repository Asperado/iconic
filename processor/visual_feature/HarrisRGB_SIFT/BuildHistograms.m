...
%H_all = zeros(length(imageFileList),params.dictionarySize);
for f = 1:length(imageFileList)
    ...
    if(exist(inFName,'file'))
      try
        ...
        fprintf('Bad sift festures %s, remove file.\n', inFName);
        delete(inFName);
        features = sp_gen_sift(fullfile(imageBaseDir, imageFName),params);
%        save(inFName, 'features');
      end
    else
      try
        features = sp_gen_sift(fullfile(imageBaseDir, imageFName),params);
      catch
	fprintf ('Retry one more time.');
        features = sp_gen_sift(fullfile(imageBaseDir, imageFName),params);
      end
%        save(inFName, 'features');
    end
    ...
    if(size(features.data,1)==1)
        disp('blank image');
        features.data = ones(1,128*3);
    end
    
    
    if ndata <= batchSize
        dist_mat = sp_dist2(double(features.data), dictionary);
        ...
        for j = 1:batchSize:ndata
            ...
            dist_mat = sp_dist2(double(features.data(lo:hi,:)), dictionary);
            [min_dist, min_ind] = min(dist_mat, [], 2);
