...
for f = 1:params.numTextonImages    
    ...
    if(exist(inFName,'file'))
        fprintf('load sift at %s\n', inFName);
        load(inFName, 'features');
    else
        fprintf('generate sift at %s\n', fullfile(imageBaseDir, imageFName));
        features = sp_gen_sift(fullfile(imageBaseDir, imageFName),params);
    end
    
    if (size(features.data, 2) ~= 384) 
        size(features.data)
        fprintf('Regenerate sift at %s\n', fullfile(imageBaseDir, imageFName));
        features = sp_gen_sift(fullfile(imageBaseDir, imageFName),params);
    end

    if (size(features.data, 2) ~= 384) 
        fprintf('%d Broken Image %s\n', broken_image_cnt, fullfile(imageBaseDir, imageFName));
        broken_image_cnt = broken_image_cnt + 1;
        continue;
    end
