...
pyramid_all = zeros(length(imageFileList),params.dictionarySize*sum((2.^(0:(params.pyramidLevels-1))).^2), 'single');
for f = 1:length(imageFileList)
    ...
    try
      ...
      delete(in_fname);
      fprint('Remove broken texton_ind file %s.', in_fname);
...
fprintf('Total bad images number %d while compiling pyramid.\n', bad_count);
